...
function FacultyList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post("http://localhost:8000/getalluser");
    ...
    const columns = [
        ...
        {
            name: "Actions",
            cell: (row) => <button className={style.removebutton} onDoubleClick={() => handleRemove(row._id, row.email)}>Remove</button>,
            button: true,
            width: "200px",
