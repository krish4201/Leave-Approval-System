...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (finduser.role == "hod" || finduser.role == "dean") {
      res.json(
        bcrypt.compare(pwd, finduser.pwd) ? ["admin", user] : ["wrong", user]
      );
    } else if (finduser.role == "faculty") {
      res.json(
        bcrypt.compare(pwd, finduser.pwd) ? ["user", user] : ["wrong", user]
      );
    } else {
      res.json("notexist");
    }
...
app.post("/applyleave", async (req, res) => {
  ...
  try {
    ...
    if (!find) {
      ...
      const insert = await leavedatas.updateOne(
        {},
        { $push: { hod: userLeaveData } }
      );
      await leavestatus.updateOne({
        email: userLeaveData.email,
        status: "Applied",
      });
...
//     await historydata.updateOne(
//       { email: history.email, applieddata: history.applieddate },
//       { status: "Accepted" }
//     );
...
app.post("/accept", async (req, res) => {
  const { email, applieddate } = req.body;

  try {
    // 1. Find the leave entry in dean array for this email + applieddate
    const doc = await leavedatas.findOne({
      "dean.email": email,
      "dean.applieddate": applieddate,
    });
    ...
    // 2. Extract the exact leave object from dean array
    const leave = doc.dean.find(
      (l) => l.email === email && l.applieddate === applieddate
    );
    ...
    // 3. Save to history with status Accepted
    const history = {
      ...leave,
      status: "Accepted",
    };
    ...
    // 5. Remove this leave from dean array
    await leavedatas.updateOne(
      { _id: doc._id },
      { $pull: { dean: { email: email, applieddate: applieddate } } }
    );
    ...
    transporter
      .sendMail({
        ...
        text: "Your leave application has been accepted",
      })
      .catch(console.error);
...
//       leavedecline = await leavedatas.deleteOne({
//         "dean.email": history.email,
//       });
//     }
...
//     res
//       .status(500)
//       .json({ error: "An error occurred while declining leave request." });
//   }
// });
...
app.post("/decline", async (req, res) => {
  ...
  try {
    ...
    // Create a history record with Declined status
    history.status = "Declined";
    await historydata.create(history);
    ...
    transporter
      .sendMail({
        ...
        to: history.email,
        subject: "Leave Application status",
        text: "Your leave application has been declined",
      })
      .catch(console.error);

    res.json(await leavedatas.find());
  } catch (error) {
    console.error(error);
    res
      .status(500)
